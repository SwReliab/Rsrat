...
List em_lxvmin_emstep(NumericVector params, List data) {
  ...
  double g02 = 1.0;
  double g10 = exp(-y);
  double g11 = exp(-loc/scale)*(1.0 + y)*exp(-y);
  double g12 = exp(-y) * (1.0 + y * log(y));
  if (x != 0.0) {
    double tmp1 = g00 - g10;
    double tmp2 = g01 - g11;
    double tmp3 = g02 - g12;
  ...
  if (type[0] == 1) {
    ...
    llf += Revd::dgumbel_min(log(t), loc, scale, true) - log(t); // log(lxvmin_pdf(t, scale, loc));
  }
