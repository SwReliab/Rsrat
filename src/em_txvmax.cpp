...
List em_txvmax_emstep(NumericVector params, List data) {
  ...
  const double omega_dash = omega / Revd::pgumbel(0, loc, scale, false, false);
  ...
  double g00 = 1.0 - exp(-y0);
  double g01 = exp(-loc/scale)*(1.0 - (1.0 + y0)*exp(-y0));
  double g02 = 1.0 - exp(-y0) * (1.0 + y0 * log(y0));
  double t = time[0];
  double x = num[0];
  ...
  double g11 = g01;
  double g12 = g02;
  double g20 = 1.0 - exp(-y);
  double g21 = exp(-loc/scale)*(1.0 - (1.0 + y)*exp(-y));
  double g22 = 1.0 - exp(-y) * (1.0 + y * log(y));
  if (x != 0.0) {
    double tmp1 = g10 - g20;
    double tmp2 = g11 - g21;
    double tmp3 = g12 - g22;
  ...
  if (type[0] == 1) {
    ...
    llf += Revd::dgumbel(t, loc, scale, true) - Revd::pgumbel(0, loc, scale, false, true); // log(txvmax_pdf(t, scale, loc));
  }
  for (int j=1; j<dsize; j++) {
    ...
    if (time[j] != 0.0) {
      ...
      g12 = g22;
      g20 = 1.0 - exp(-y);
      g21 = exp(-loc/scale)*(1.0 - (1.0 + y)*exp(-y));
  ...
  double new_omega =  en1 * (1.0 - exp(-y0));
